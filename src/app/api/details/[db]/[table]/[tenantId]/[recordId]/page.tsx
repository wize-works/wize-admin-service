...
export default async function RecordDetailsPage({ params }: { params: { db: string; table: string; identityId: object; recordId: object; } }) {
  const { db, table, identityId, recordId } = params;
  ...
  if (!record) {
    return (
      <div className="p-5">
        ...
        <Link href={`/fields?db=${db}&table=${table}&option=${recordId}`} className="text-blue-600 hover:underline mt-4 inline-block">
          Back to Table
        </Link>
      </div>
    );
  ...
  return (
    <div className="p-5">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href={`/fields?db=${db}&table=${table}&option=${recordId}`} className="text-blue-600 hover:underline">
          Back to Table
        </Link>
      </div>
          ...
          <dl className="divide-y divide-gray-200">
            {Object.entries(record).map(([field, value]) => (
              <div key={field} className="py-4 flex justify-between">
                <dt className="text-gray-600 font-medium w-1/3">{field}</dt>
                <dd className="text-gray-900 w-2/3">
                  {typeof value === 'object' 
